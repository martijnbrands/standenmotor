              ...
              <v-layout wrap pt-3>
                ...
                <v-flex>
                  <strong class="d-block pb-3 font-weight-bold">{{ match.homeTeam }}</strong>
                    <div
                      v-for="driver in match.players"
                      :key="driver.id"
                      class="mb-2"
                    >{{ driver.name }}</div>
                </v-flex>
                <v-flex v-if="match.awayUniform">
                 <v-icon border="1px solid black" color="#ffd907">mdi-tshirt-crew</v-icon>
                </v-flex>
              </v-layout>
              <div v-if="$auth.user().account_type === 'Team Admin'">
                <v-select
                  class="d-block"
                  clearable
                  :items="players"
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    small
                    color="primary"
                    @click="addDrivers(match, match.players, index);"
                  >Opslaan</v-btn>
                </v-card-actions>
              </div>
            </v-timeline-item>
...
export default {
  ...
  methods: {
    ...
    updateMatches(checkedMatches) {
      axios
        .post("/matches/create", checkedMatches)
        .then(response => {
